...
// Null returns the parser always generating null result.
func Null() Parser {
	return &nullParser{}
}
...
// OrResultValue is wrapper of the result value generated by OrWithIndex.
type OrResultValue interface {
	Index() int
	Value() interface{}
}

type orResultValue struct {
	i int
	v interface{}
}

func (v *orResultValue) Index() int {
	return v.i
}

func (v *orResultValue) Value() interface{} {
	return v.v
}
...
func (p *orParser) Parse(r SourceReader) (ParseResult, error) {
	...
	for i, child := range p.children {
		...
		if p.withIndex {
			result = NewParseResult(ret.SourceRange(), &orResultValue{i, ret.Value()})
		} else {
			result = ret
		}
		break
...
func Or(ps ...Parser) Parser {
	return &orParser{ps, false}
}

func OrWithIndex(ps ...Parser) Parser {
	return &orParser{ps, true}
}
...
func (p *manyParser) String() string {
	return fmt.Sprintf("Many[%d-%d](%s)", p.min, p.max, p.child.String())
}
...
func MapValue(p Parser, fn func(v interface{}) (interface{}, error)) Parser {
	return Map(p, func(r ParseResult) (ParseResult, error) {
		v, err := fn(r.Value())
		if err != nil {
			return nil, NewParseError(r.SourceRange(), err.Error())
		}
		return NewParseResult(r.SourceRange(), v), nil
	})
}

func MapValueAsResults(p Parser, fn func(results []ParseResult) (interface{}, error)) Parser {
	return Map(p, func(r ParseResult) (ParseResult, error) {
		rs, ok := r.Value().([]ParseResult)
		if !ok {
			return nil, NewParseError(r.SourceRange(), "not array of result value.")
		}
		v, err := fn(rs)
		if err != nil {
			return nil, NewParseError(r.SourceRange(), err.Error())
...
func Str(s string) Parser {
	...
	return ConcatStr(And(ps...))
}

func StrByAnd(ps ...Parser) Parser {
	return ConcatStr(And(ps...))
}

func StrByRuneIn(s string, minLen int) Parser {
	return ConcatStr(ManyN(RuneIn(s), minLen))
}

func StrByRuneNotIn(s string, minLen int) Parser {
...
func ManyWithoutNil(p Parser) Parser {
	return FilterNil(Many(p))
}
